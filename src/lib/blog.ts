/**
 * Blog content — kept as TypeScript data rather than MDX so it stays
 * portable without adding a content pipeline (Phase 1 trade-off).
 * Authoring is done in code; in Phase 2 this can move to MDX or a
 * CMS-backed source without changing the route signature.
 */
export type BlogPost = {
  slug: string;
  title: string;
  excerpt: string;
  category: "guides" | "actualites" | "destinations";
  publishedAt: string; // ISO
  readingMinutes: number;
  cover: string;        // /public path
  author: string;
  body: string[];       // paragraphs (split by blank line)
};
...
export const blogPosts: BlogPost[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "Conseil pratique : emportez un词典 français-arabe pour les premiers jours et un adaptateur secteur type G (UK) — l'Arabie saoudite n'a pas migré vers le type C universel.",
    ],
  },
  {
